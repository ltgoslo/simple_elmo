...
def print_variable_summary():
    ...
    variables = sorted(
        [[v.name, v.get_shape()] for v in tf.compat.v1.global_variables()]
    )
...
class LanguageModel(object):
    ...
    def _build_word_embeddings(self):
        ...
        self.token_ids = tf.compat.v1.placeholder(
        ...
        with tf.device("/cpu:0"):
            ...
            self.embedding = tf.nn.embedding_lookup(
                params=self.embedding_weights, ids=self.token_ids
            )

        # if a bidirectional LM then make placeholders for reverse
        # model and embeddings
        if self.bidirectional:
            self.token_ids_reverse = tf.compat.v1.placeholder(
    ...
    def _build_word_char_embeddings(self):
        ...
        self.tokens_characters = tf.compat.v1.placeholder(
            DTYPE_INT,
        ...
        with tf.device("/cpu:0"):
            ...
            self.char_embedding = tf.nn.embedding_lookup(
                params=self.embedding_weights, ids=self.tokens_characters
            )

            if self.bidirectional:
                self.tokens_characters_reverse = tf.compat.v1.placeholder(
                    DTYPE_INT,
                    shape=(batch_size, unroll_steps, max_chars),
        ...
        # the convolutions
        def make_convolutions(inp, reuse_val):
            with tf.compat.v1.variable_scope("CNN", reuse=reuse_val) as conv_scope:
                convolutions = []
                for nr_i, (width, num) in enumerate(filters):
                    ...
                    conv = (
                        tf.nn.conv2d(
                            input=inp, filters=w, strides=[1, 1, 1, 1], padding="VALID"
                        )
                        + b
                    )
                    # now max pool
        ...
        if use_highway:
            ...
            for i in range(n_highway):
                ...
                embedding = high(
                    embedding, weights_carry, b_carry, weights_transform, b_transform
                )
                if self.bidirectional:
                ...
                self.token_embedding_layers.append(
                    tf.reshape(embedding, [batch_size, unroll_steps, highway_dim])
                )
        ...
        if use_proj:
            ...
            if self.bidirectional:
                embedding_reverse = (
                    tf.matmul(embedding_reverse, weights_proj_cnn) + b_proj_cnn
                )
            self.token_embedding_layers.append(
    ...
    def _build_loss(self, lstm_outputs):
        ...
        def _get_next_token_placeholders(suffix):
            ...
            idplaceholder = tf.compat.v1.placeholder(
                DTYPE_INT, shape=(batch_size, unroll_steps), name=name
            )
        ...
        with tf.compat.v1.variable_scope('softmax'):
            ...
            softmax_init = tf.compat.v1.random_normal_initializer(
        ...
        for id_placeholder, lstm_output_flat in zip(next_ids, lstm_outputs):
            ...
            with tf.control_dependencies([lstm_output_flat]):
                if self.is_training and self.sample_softmax:
                    ...
                    output_scores = tf.transpose(
                        tf.matmul(self.softmax_W, tf.transpose(lstm_output_flat))) + self.softmax_b
...
def _deduplicate_indexed_slices(values, indices):
    ...
    summed_values = tf.math.unsorted_segment_sum(
        values, new_index_positions, tf.shape(input=unique_indices)[0]
    )
...
def train(options, data, n_gpus, tf_save_dir, tf_log_dir, restart_ckpt_file=None):
    ...
    with tf.device("/cpu:0"):
        ...
        for v in tf.compat.v1.global_variables():
            histogram_summaries.append(
                tf.compat.v1.summary.histogram(v.name.replace(":", "_"), v)
            )
    ...
    with tf.compat.v1.Session(
        ...
        for batch_no, batch in enumerate(data_gen, start=1):
            ...
            for k in range(n_gpus):
                ...
                feed_dict.update(
                    _get_feed_dict_from_x(
                        x, start, end, model, char_inputs, bidirectional
                    )
                )
            ...
            if batch_no % 1250 != 0:
                ret = sess.run(
                    [train_op, summary_op, train_perplexity] + final_state_tensors,
                    feed_dict=feed_dict,
                )
            ...
            if batch_no % 100 == 0:
                ...
                print(
                    "Batch %s out of %s, train_perplexity=%s"
...
def test(options, ckpt_file, data, batch_size=256):
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        for batch_no, batch in enumerate(data.iter_batches(batch_size, 1), start=1):
            ...
            feed_dict.update(
                _get_feed_dict_from_x(
                    x, 0, x["token_ids"].shape[0], model, char_inputs, bidirectional
                )
            )
            ret = sess.run(
                [model.total_loss, final_state_tensors, model.output_scores],
                feed_dict=feed_dict,
            )
            ...
            print(
                "batch=%s, batch_perplexity=%s, avg_perplexity=%s, time=%s"
...
def lm_encode(sentences_raw, vocab, reverse=False):
    if reverse:
        sentences = []
    ...
    ids = [vocab.encode(sentence, reverse=reverse) for sentence in sentences]
    if hasattr(vocab, "encode_chars"):
        chars_ids = [
            vocab.encode_chars(sentence, reverse=reverse) for sentence in sentences
        ]
    else:
        chars_ids = [None] * len(ids)
    for encoded in zip(ids, chars_ids):
        yield encoded
...
def load_options_latest_checkpoint(tf_save_dir):
    ...
    if not ckpt_file:
        ckpt_file = tf_save_dir + "model.ckpt"
...
def dump_weights(tf_save_dir, outfile):
    ...
    def _get_outname(tf_name):
        outfilename = re.sub(":0$", "", tf_name)
        outfilename = outfilename.lstrip("lm/")
        outfilename = re.sub("/rnn/", "/RNN/", outfilename)
        outfilename = re.sub("/multi_rnn_cell/", "/MultiRNNCell/", outfilename)
        outfilename = re.sub("/cell_", "/Cell", outfilename)
        outfilename = re.sub("/lstm_cell/", "/LSTMCell/", outfilename)
        if "/RNN/" in outfilename:
            if "projection" in outfilename:
                outfilename = re.sub("projection/kernel", "W_P_0", outfilename)
            else:
                outfilename = re.sub("/kernel", "/W_0", outfilename)
                outfilename = re.sub("/bias", "/B", outfilename)
        return outfilename
